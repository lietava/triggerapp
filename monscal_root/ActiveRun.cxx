...
int ActiveRun::ProcessCfgLine(const string &line,int& level)
{
 ...
 switch (level){
   ...
   case 2:  //interactions
          //cout << "INTERACTION found." << endl;
          /*
          if(items[0].find("INT1") != string::npos){
            string name;
            name="IR1:"+items[1];
            fINT->SetName1(name);
          }else if(items[0].find("INT2") != string::npos){
            string name;
            name="IR2:"+items[1];
            fINT->SetName2(name);
          }else{
            cout << "Error: Processcfgline: wrong int:" << endl;
            cout << line << endl;
          }
          */
          return 0;
...
// toto je nejaka blbost, treba opravit
 ifstream file;
 stringstream ss;
 if(runnumber){  
    if(machine == '7'){ // RCFG is not visible from 027=>copy
      ss << "scp -p trigger@alidcscom188:v/vme/WORK/RCFG/r" <<runnumber << ".rcfg RCFG/.";
      int rc=system((ss.str()).c_str());
      if(rc){
       cout << "ActiveRun::ParseConfigFile : cannot copy " << ss.str() << endl;
       return 1;
      }
      ss.str("");
      ss << "RCFG/r" <<runnumber << ".rcfg";
      frcfgfile = ss.str();
    }else{ // use local copy on 05 for debug
      ss << "RCFG/r" <<runnumber << ".rcfg";
      frcfgfile = ss.str(); 
    }
 }else{  // inputs.rcfg is in VMECFDIR on all machines
   char* path = getenv("VMECFDIR");
   if(path == 0){
     cout << "VMECFDIR not defined, taking pwd/inputs.rcfg" << endl;
     frcfgfile = "RCFG/inputs.rcfg";
   }else{
